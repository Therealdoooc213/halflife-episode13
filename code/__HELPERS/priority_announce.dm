...
/proc/priority_announce(text, title = "", sound, type, sender_override, has_important_message = FALSE, list/mob/players = GLOB.player_list, encode_title = TRUE, encode_text = TRUE, color_override)
	...
	switch(type)
		...
		if(ANNOUNCEMENT_TYPE_CAPTAIN)
			...
			GLOB.news_network.submit_article(text, "Captain's Announcement", "District Announcements", null)
	...
	if(isnull(sender_override) && players == GLOB.player_list)
		if(length(title) > 0)
			GLOB.news_network.submit_article(title + "<br><br>" + text, "[command_name()]", "District Announcements", null)
		else
			GLOB.news_network.submit_article(text, "[command_name()] Update", "District Announcements", null)
